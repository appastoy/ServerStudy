...
namespace Neti.CodeGeneration
{
	static class TypeUtility
	{
		...
		public static string GetUnfriendlyParameterTypeUsingCode(IEnumerable<MethodInfo> methods, string exceptNamespace = null, IEnumerable<string> additionalUsings = null)
		{
			var paramTypes = methods.SelectMany(method => method.GetParameters().Select(param => param.ParameterType))
									.Distinct();

			return GetUnfriendlyTypeUsingCode(paramTypes, exceptNamespace, additionalUsings);
		}

		public static string GetUnfriendlyTypeUsingCode(IEnumerable<Type> types, string execptNamespace = null, IEnumerable<string> additionalUsings = null)
		{
			if (types is null)
			{
			...
			var usingNamespaces = types.Where(type => IsFriendlyType(type) == false)
									   ...
									   .Concat(additionalUsings ?? Array.Empty<string>())
									   .Distinct()
